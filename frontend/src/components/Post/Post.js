...
const Post = ({post}) => {
    ...
    return (
        <>
            <Card style={{width: "600px"}}>
                <CardBody>
                    <div className="row">
                        ...
                        />
                        <div className="col-4">{post?.user.first_name} {post?.user.last_name}</div>
                        <div className={"col-2"}>@{post?.user.username}</div>
                        <div className={"col-4"}>{moment(post?.creation_date).format("DD.MM.YYYY")}</div>
                        {post.is_owner && <div className="col-1 float-right"><img className="post-more-icon" src={require('assets/icons/more.svg')} alt=""/></div>}
                         {/*TODO drop down*/}
                    </div>
                    <pre className="post-description ml-5">
                    {post?.description}
                </pre>
                    <div className="row">
                        ...
                        <i className="nc-icon nc-chat-33 ml-4 post-cursor-pointer" style={{fontSize: "25px"}} onClick={toggleCommentModal}/>
                        <a className="post-like-comment ml-2 post-cursor-pointer" onClick={toggleCommentsListModal}>{postComments}</a>
                    </div>
            ...
            {comments && <Modal isOpen={commentsListModal} toggle={toggleCommentsListModal} centered={true} style={{maxHeight: "80%"}}>
